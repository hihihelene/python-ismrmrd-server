...
def process_image(imgGroup, connection, config, mrdHeader):
    ...
    logging.debug("Processing data with %d images of type %s", len(imgGroup), ismrmrd.image.get_dtype_from_data_type(imgGroup[0].data_type))
    ...
    #         return []
        ...
        # VQMaps = VQMaps.astype(np.float32)
        # VQMaps -= VQMaps.min()
        # VQMaps *= 1/VQMaps.max()

        # Apply colormap
        # cmap = plt.get_cmap('jet')
        # VQMaps_rgb = cmap(VMap)

        # Remove alpha channel
        # Resulting shape is [row col z rgb img]
        # VMap_rgb = VMap_rgb[...,0:-1]
        # VMap_rgb = VMap_rgb.transpose((0, 1, 2, 5, 4, 3))
        # VMap_rgb = np.squeeze(VMap_rgb, 5)

        # MRD RGB images must be uint16 in range (0, 255)
        # VQMaps *= 255
        # VQMaps = VQMaps.astype(np.uint16)
        # np.save(debugFolder + "/" + "imgVMapRGB.npy", VQMaps)
    ...
    if mrdhelper.get_json_config_param(config, 'sendOriginal', default=True, type='bool') == True:
